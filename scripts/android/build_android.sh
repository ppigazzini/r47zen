...
ensure_retired_legacy_cpp_paths_absent() {
    ...
    if [ -n "$present_paths" ]; then
        ...
        printf '%s\n' "$present_paths" >&2
        fail "Retired app-module native snapshot paths must stay absent. Use $STAGED_CPP_DIR for staged Android inputs, $ANDROID_PROJECT_DIR/app/src/main/cpp/r47zen for Android-owned glue, and $MINI_GMP_FALLBACK_DIR for the tracked Android mini-gmp staging source."
    fi
}
...
if R47_CMAKE_BIN=$(resolve_cmake_bin); then
    R47_CMAKE_BIN_DIR=$(dirname "$R47_CMAKE_BIN")
    export PATH="$R47_CMAKE_BIN_DIR:$PATH"
else
    R47_CMAKE_BIN=""
fi
...
PGO_PROFILE_PATH_OVERRIDE=${R47_PGO_PROFILE_PATH-}
