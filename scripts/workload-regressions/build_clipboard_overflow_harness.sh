#!/bin/bash
# Build and run the clipboard register-dump overflow harness under AddressSanitizer
...
BUILD_DIR="$PROJECT_ROOT/android/build/clipboard-overflow-harness"
CC_BIN="${CC:-cc}"
mkdir -p "$BUILD_DIR"
...
"$CC_BIN" -std=gnu11 -O1 -g -pthread \
    ...
    "$SCRIPT_DIR/clipboard_overflow_harness.c" \
    "${STAGED_C47_SOURCES[@]}" \
    ...
    "${ANDROID_BRIDGE_SOURCES[@]}" \
    -lm \
    -o "$BUILD_DIR/clipboard-overflow-harness"

echo "Built: $BUILD_DIR/clipboard-overflow-harness"
echo "Running under ASan/UBSan..."
ASAN_OPTIONS="abort_on_error=1:halt_on_error=1" \
    UBSAN_OPTIONS="print_stacktrace=1:halt_on_error=0" \
    "$BUILD_DIR/clipboard-overflow-harness"
