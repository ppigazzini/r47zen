...
| stop delivery during long-running program execution | `MainActivity.kt`, `LiveProgramStopKeyPolicy.kt`, `ReplicaOverlayController.kt`, `NativeDisplayRefreshLoop.kt`, `NativeKeypadSnapshotStore.kt`, `NativeCoreRuntime.kt`, `jni_display.c`, `jni_input.c`, `android_runtime.c` | `LiveProgramStopKeyPolicyTest.kt`, `NativeDisplayRefreshLoopTest.kt`, `ReplicaOverlayControllerLabelModeTest.kt`, focused JVM stop-routing ownership checks, plus manual Android `MANSLV2.p47` evidence until a connected-device interrupt lane lands | rerun `LiveProgramStopKeyPolicyTest` first, then the focused JVM lane, then use manual device evidence or a future connected instrumentation lane when the Android-owned stop routing changes |
...
- `scripts/workload-regressions/run_workload_regressions.sh` builds the staged
  ...
  runs the canonical `BinetV3.p47`, `GudrmPL.p47`, `NQueens.p47`, and
