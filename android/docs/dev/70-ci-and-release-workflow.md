...
- uses a monotonic positive-integer `version_code`; the maintainer convention is
  `YYYYMMDDVV`, for example `2026052701`
- uses a signed-release `version_name` as both the user-facing bundle version
  and the GitHub release tag basis; the maintainer convention is
  `major.minor.patch-signed.YYYYMMDD`, for example
  `0.1.0-signed.20260527`
...
- keeps the derived tag predictable when `version_name` stays ASCII and follows
  the recommended dotted signed-release pattern
