...
class DisplayLifecycleInstrumentedTest {
    ...
    fun pauseResumePreservesInjectedDisplaySnapshot() {
        // DE-FLAKE (REPORT-24 Milestone 4b Slice C): the former
        // pauseResumePreservesSpiralkGraphSnapshot ran SPIRALk emergently only to
        // get a non-trivial framebuffer before asserting a Settings-style
        // pause/resume preserves it. The pause/resume transition is display-passive
        // -- the core thread keeps running and packedDisplayBuffer is not
        // re-rendered (NativeCoreRuntime's shared init flags mean onResume
        // re-attaches without re-init) -- so the contract holds for ANY
        // framebuffer. A deterministic non-trivial pattern is injected (its hash
        // captured atomically), the lifecycle event runs, and the framebuffer is
        // re-hashed, with no program run and no timeout.
    ...
    @Test
    fun activityRecreationPreservesInjectedDisplaySnapshot() {
        // DE-FLAKE (REPORT-24 Milestone 4b Slice C): the former
        // activityRecreationPreservesSpiralkGraphSnapshot ran SPIRALk emergently
        // only to get a non-trivial framebuffer before asserting recreation
        // preserves it. Recreation is display-passive: NativeCoreRuntime's
        // isCoreThreadStarted/isNativeInitializedShared are process-shared, so a
        // recreated MainActivity re-attaches (updateNativeActivityRef) without
        // re-initialising or re-rendering -- the native runtime and
        // packedDisplayBuffer persist. So the contract holds for ANY framebuffer;
        // a deterministic pattern is injected, recreation runs, and the
        // framebuffer is re-hashed, with no program run and no timeout.
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            assertTrue(
                "Native runtime did not become ready for recreation coverage",
                waitUntil(RUNTIME_READY_TIMEOUT_MS) { ProgramLoadTestBridge.isRuntimeReady() },
            )
            ProgramLoadTestBridge.resetRuntime()
            ...
            assertEquals(
                "Activity recreation should preserve the display snapshot",
                beforeHash,
                afterHash,
            )
        }
